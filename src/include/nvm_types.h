 ...
 * Device descriptor
 */
//这是用来与 GeminiFS 定制的 CPU/GPU 共享 NVMe 驱动 (SNVMe) 通信的句柄 。
struct device
{
 ...
 */
//这是整个系统最核心的结构体之一，用来描述 NVMe 的提交队列 (SQ) 和完成队列 (CQ)。
typedef struct
{
 ...
 */
//是 NVMe 协议标准的物理区域页 (PRP) 列表。当你需要读取的数据在物理内存中不连续时，
//程序会构造一个 PRP 列表交给 SSD 控制器，SSD 就会利用 DMA 引擎自动把数据搬运到这些物理地址（GPU 显存）里。
typedef struct __align__(32)
{
 ...
 */
//这些结构体用于在系统初始化（Geminifs_init）时，保存从硬件读取出来的属性 。程序后续计算偏移量、限制 I/O 大小都会依赖这些只读信息。
//下面的都是
